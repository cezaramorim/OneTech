...
# ðŸ“ Unidade de Medida (UN, KG, CX, etc.)
...
# ðŸ›’ Produto principal â€” agora usado tambÃ©m como item da nota
class Produto(models.Model):
    # ðŸ”‘ IdentificaÃ§Ã£o
    codigo_interno = models.CharField(max_length=50, unique=True, editable=False, help_text="CÃ³digo Ãºnico interno do produto (gerado automaticamente).")
    codigo_fornecedor = models.CharField(max_length=50, blank=True, null=True, help_text="CÃ³digo do produto conforme fornecedor.")
    ...
    tipo = models.CharField(
        ...
        default='Produto',
        help_text="ClassificaÃ§Ã£o do produto para fins operacionais."
    )
    ...
    # ðŸ§¾ Dados fiscais complementares
    codigo_barras = models.CharField(max_length=50, blank=True, null=True, help_text="CÃ³digo de barras (EAN)")
    ...
    # ðŸ§¾ Nota fiscal de origem (para importaÃ§Ã£o XML)
    nota_fiscal = models.ForeignKey(
        'nota_fiscal.NotaFiscal',
        on_delete=models.CASCADE,
        null=True, blank=True,
        related_name='produtos',
        help_text="Nota fiscal Ã  qual este produto estÃ¡ vinculado (caso importado do XML)"
    )

    # ðŸ”„ ConversÃ£o de Unidade
    fator_conversao = models.DecimalField(
        max_digits=18,
        decimal_places=10,
        default=1,
        help_text="Fator para converter a unidade de compra para a unidade interna. Ex: Se compra em CX com 12 UN, o fator Ã© 12."
    )
    unidade_fornecedor_padrao = models.ForeignKey(UnidadeMedida, on_delete=models.SET_NULL, null=True, blank=True, related_name='produtos_unidade_fornecedor', help_text="Unidade de medida padrÃ£o usada pelo fornecedor para este produto.")
    ...
    def recalculate_stock_and_prices(self):
        ...
        from produto.models_entradas import EntradaProduto # ImportaÃ§Ã£o local para evitar circular dependency
        ...
        for entrada in entradas:
            # Aplica o fator de conversÃ£o atual do produto
            ...
            print(f"DEBUG: Entrada {entrada.pk}: Quantidade XML: {entrada.quantidade}, PreÃ§o UnitÃ¡rio XML: {entrada.preco_unitario}, Quantidade Convertida: {quantidade_convertida}, PreÃ§o Convertido: {preco_unitario_convertido}")
        ...
        if total_quantidade_entradas > 0:
            ...
            # O preco_custo pode ser o preco_medio ou o preco_unitario da Ãºltima entrada,
            # dependendo da regra de negÃ³cio. Por simplicidade, usaremos o preco_medio.
        ...
        print(f"DEBUG: Valores recalculados para {self.nome}: Estoque Total: {self.estoque_total}, PreÃ§o MÃ©dio: {self.preco_medio}, PreÃ§o Custo: {self.preco_custo}")
    ...
    def save(self, *args, **kwargs):
        # Gerar codigo_interno sequencial se nÃ£o existir e o produto for novo
        if not self.pk and not self.codigo_interno:
            from django.db.models import Max
            from django.db.models.functions import Cast
            from django.db.models import IntegerField

            # Encontra o maior cÃ³digo numÃ©rico existente
        ...
        print(f"DEBUG: MÃ©todo save do Produto chamado para {self.nome} (ID: {self.pk})")
        # Verifica se o fator_conversao foi alterado
        if self.pk: # Se o objeto jÃ¡ existe no banco de dados
            try:
                original = Produto.objects.get(pk=self.pk)
                if original.fator_conversao != self.fator_conversao:
                    print(f"DEBUG: Fator de conversÃ£o alterado de {original.fator_conversao} para {self.fator_conversao}. Recalculando estoque e preÃ§os.")
                    # Salva primeiro para garantir que o fator_conversao esteja atualizado
                    super().save(*args, **kwargs) 
                    self.recalculate_stock_and_prices()
                    return # JÃ¡ salvou e recalculou, entÃ£o retorna
            except Produto.DoesNotExist:
                print(f"DEBUG: Produto com ID {self.pk} nÃ£o encontrado no banco de dados. Prosseguindo com save normal.")
                pass # Objeto nÃ£o existe, Ã© uma criaÃ§Ã£o, prossegue com o save normal
